...
import { Url } from "../constant.js/PatientConstant";
  ...
  export const editDoctor = (id, name, role, email, phone, registration_no) => async (dispatch, getState) => {
    dispatch({ type: EDIT_DOCTOR_REQUEST });
    const { adminSignin: { adminDocInfo } } = getState();
  
    try {
      const { data } = await axios.put(`${Url}/doctors/edit/${id}`, { name, role, email, phone, registration_no }, {
        headers: {
          Authorization: `Bearer ${adminDocInfo.token}`,
        },
      });
